...
std::string create_line(const Student s) {
...
void write_data(const std::string file_name,
               std::map< std::string, Student* >& alphabetical_order) {
    std::ofstream file_object("temp.txt");

    for(auto pair: alphabetical_order) {
        file_object << create_line(*(pair.second)) << std::endl;
    }
    file_object.close();
...
void change_phone_number (Student & s, std::string phone) {
...
int main() {
    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            std::string number = parts[1];

            if(student_numbers.find(number) == student_numbers.end()) {
                ...
                if(is_valid_phone_number(phone)) {
                    Student *ip = student_numbers[number];
                    change_phone_number(*ip, phone);
